...
# run_freq_smnist.sh : freq_analysis(s-MNIST) - nohup 1회 실행 (python 내부에서 모델들 직렬 실행)
...
RUN_PY="${ROOT_DIR}/src/freq_analysis/s-mnist/run.py"
...
GPU="${GPU:-0}"
RUN_PREFIX="${RUN_PREFIX:-freq_smnist}"
SEED="${SEED:-0}"
...
ANALYSIS_EVERY="${ANALYSIS_EVERY:-5}"
CONV_EVERY="${CONV_EVERY:-5}"

# Optional lists (space-separated).
# - ANALYSIS_NEURONS defaults to "5 5 ..." (5 per hidden layer) when not provided.
...
EXP_PREFIX="${RUN_PREFIX}_seed${SEED}_Smax${S_MAX}"
