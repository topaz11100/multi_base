...
# run_acc_scifar10.sh : acc_benchmark(sCIFAR10) - nohup 1회 실행
...
RUN_PY="${ROOT_DIR}/src/acc_benchmark/s-cifar10/run.py"
...
GPU="${GPU:-0}"
RUN_PREFIX="${RUN_PREFIX:-acc_scifar10}"
SEED="${SEED:-0}"

MODELS_RAW="${MODELS:-all}"
MODELS_RAW="${MODELS_RAW//,/ }"
CIFAR_MODE="${CIFAR_MODE:-parallel}"
...
EXP_NAME="${RUN_PREFIX}_sCIFAR10_${CIFAR_MODE}_models${MODELS_TAG}_Smax${S_MAX_TAG}_seed${SEED}"
