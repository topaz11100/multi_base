    ...
    def fit(
        ...
        for epoch in range(start_epoch, self.max_epoches):
            ...
            for data, label in loader:
                ...
                # reset_states(model=self.network.emb.embed_tclif)
                # reset_states(model=self.network.blocks[0].attn.q_tclif)
                # reset_states(model=self.network.blocks[0].attn.k_tclif)
                # reset_states(model=self.network.blocks[0].attn.v_tclif)
                # reset_states(model=self.network.blocks[0].attn.attn_tclif)
                # reset_states(model=self.network.blocks[0].attn.last_tclif)
                # reset_states(model=self.network.blocks[0].mlp.mlp_tclif1)
                # reset_states(model=self.network.blocks[0].mlp.mlp_tclif2)
                # reset_states(model=self.network.blocks[1].attn.q_tclif)
                # reset_states(model=self.network.blocks[1].attn.k_tclif)
                # reset_states(model=self.network.blocks[1].attn.v_tclif)
                # reset_states(model=self.network.blocks[1].attn.attn_tclif)
                # reset_states(model=self.network.blocks[1].attn.last_tclif)
                # reset_states(model=self.network.blocks[1].mlp.mlp_tclif1)
                # reset_states(model=self.network.blocks[1].mlp.mlp_tclif2)
                # reset_states(model=self.network.blocks[2].attn.q_tclif)
                # reset_states(model=self.network.blocks[2].attn.k_tclif)
                # reset_states(model=self.network.blocks[2].attn.v_tclif)
                # reset_states(model=self.network.blocks[2].attn.attn_tclif)
                # reset_states(model=self.network.blocks[2].attn.last_tclif)
                # reset_states(model=self.network.blocks[2].mlp.mlp_tclif1)
                # reset_states(model=self.network.blocks[2].mlp.mlp_tclif2)
                # reset_states(model=self.network.blocks[3].attn.q_tclif)
                # reset_states(model=self.network.blocks[3].attn.k_tclif)
                # reset_states(model=self.network.blocks[3].attn.v_tclif)
                # reset_states(model=self.network.blocks[3].attn.attn_tclif)
                # reset_states(model=self.network.blocks[3].attn.last_tclif)
                # reset_states(model=self.network.blocks[3].mlp.mlp_tclif1)
                # reset_states(model=self.network.blocks[3].mlp.mlp_tclif2)
                # reset_states(model=self.network.emb)
                # reset_states(model=self.network.blocks)

                # Avoid error  TS-TCN !!
                # reset_states(model=self.network.network[0].tclif1)
                # reset_states(model=self.network.network[0].tclif2)
                # reset_states(model=self.network.network[0].tclif)
                # reset_states(model=self.network.network[1].tclif1)
                # reset_states(model=self.network.network[1].tclif2)
                # reset_states(model=self.network.network[1].tclif)
                # reset_states(model=self.network.network[2].tclif1)
                # reset_states(model=self.network.network[2].tclif2)
                # reset_states(model=self.network.network[2].tclif)
                # reset_states(model=self.network.network[3].tclif1)
                # reset_states(model=self.network.network[3].tclif2)
                # reset_states(model=self.network.network[3].tclif)
